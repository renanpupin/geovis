import React, {useEffect, useState, useCallback, useMemo, FC, useRef, memo} from 'react'
...
const MarkerList: FC<MarkerListProps> = memo(
    props => {
        ...
        const markerColorVis = useMemo(() => {
            return visualizations.filter(
                visualization =>
                    visualization.visible &&
                    visualization.type === VisualizationTypeValues.MarkerColor
            )
        }, [visualizations])
            ...
            renderer: {
                render(cluster: Cluster, stats: ClusterStats, map: google.maps.Map) {
                    ...
                    if (markerChartVis?.length > 0) {
                        ...
                        resultMarkerChartCluster = getMarkerChartImage(
                            rowsInCluster,
                            false,
                            150,
                            150
        ...
        const getMarkers = useCallback(() => {
            console.debug('=> MarkerList getMarkers render')
            ...
            return markersVisible.map((row: any, index: number) => {
                return (
                    <Marker
                        ...
                        // enableMarkerCluster={markerClusterVis.length > 0 || markersVisible?.length > 5}
    ...
    (prevProps, nextProps) => {
        return (
            !!prevProps.map === !!nextProps.map && prevProps.visibleRows === nextProps.visibleRows
        )
    }
)
