...
const StepChartAttribute: React.FC<StepChartAttributeProps> = props => {
    ...
    const [chartLabelAttribute, setChartLabelAttribute] = useState<string | undefined>(
        data?.chartLabelAttribute ?? undefined
    )
    const [chartAttributeX, setChartAttributeX] = useState<string | undefined>(
        data?.chartAttributeX ?? undefined
    )
    const [chartAttributeY, setChartAttributeY] = useState<string | undefined>(
        data?.chartAttributeY ?? undefined
    )
        ...
        <div>
            {data.chartType !== 'scatter' && (
                <div style={{marginBottom: 15}}>
                    <div style={{marginBottom: 5}}>
                        <label>
                            Select the chart {data.chartType === 'pie' ? 'group' : 'label'}{' '}
                            attribute:
                        </label>
                    </div>
                    <Select
            ...
            {data.chartType === 'scatter' && (
                <div style={{marginBottom: 15}}>
                    <div style={{marginBottom: 5}}>
                        <label>
                            Select the chart numeric attribute
                            {data.chartType === 'scatter' ? ' x' : ''}:
                        </label>
                    </div>
                    <Select
            ...
            {(data.chartType !== 'pie' || (data.chartType === 'pie' && !data.hasToGroup)) && (
                <div style={{marginBottom: 15}}>
                    <div style={{marginBottom: 5}}>
                ...
                <div style={{marginBottom: 15}}>
                    ...
                    <div>
                        ...
                        <span style={{display: 'block', marginTop: 5, fontSize: 12}}>
                            * Attribute numeric value will be used instead of number of ocurrences
                        </span>
                    </div>
                </div>
            )}
